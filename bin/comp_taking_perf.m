...
algs = ["ac_tc", "dpg", "ddpg"];
alg = algs(ia);
tbl_meanstd_all = generate_tbl(env, env_abr, alg, withLoad, exc);
...
function tbl_meanstd_all = generate_tbl(env, env_abr, alg, withLoad, exc)
    steps_counted = 10;
    printing = 1;
    folder = "~/Dropbox/phd_grl_results/phd_grl_mpol_results/";
    addpath("~/Dropbox/phd_grl_results/matlab");
    ...
    for ig = 1:n_group
        for il = 1:n_load
            ...
            i = (ng*n_load) + nl*(ig-1) + 2*il -1;
            j = (ng*n_load) + nl*(ig-1) + 2*il;
	...
	if (exc)
            ...
            tbl_meanstd_specific_good = test_take_mean_mpol(folder, env, env_abr, "", alg, runs_specific_good, printing, steps_per_second, steps_counted);
            ...
            runs_specific_mid = [         type + "_*_mean_euclidian_distance_0.5_density_a0.01_*.txt",...
                                  ...
                                  "r25" + type + "_*_mean_euclidian_distance_0.5_density_a0.01_*.txt"];
            tbl_meanstd_specific_mid = test_take_mean_mpol(folder, env, env_abr, "", alg, runs_specific_mid, printing, steps_per_second, steps_counted);
            ...
            runs_specific_bad = [         type + "_*_none_data_center_linear_order_0.25_data_center_a0.01_*txt",...
                                  ...
                                  "r25" + type + "_*_mean_euclidian_distance_0.5_density_a0.01_*.txt",...
                          "r50" + type + "_*_mean_euclidian_distance_0.5_density_a0.01_*.txt"];
        tbl_meanstd_specific_bad = test_take_mean_mpol(folder, env, env_abr, "", alg, runs_specific_bad, printing, steps_per_second, steps_counted);
