...
for it=init_size_env:size_env
    ...
    [tbl_meanstd_all, percentual, strategies] = generate_tbl_article(root, printing, onlybad4pend, steps_counted, runs_number);
...
function [tbl_meanstd_all, percentual, strategies] = generate_tbl_article(root, printing, onlybad4pend, steps_counted, runs_number)

    group = ["pendulum_pd_", "cart_pole_cp", "cart_double_pole_cdp", "half_cheetah"];
    ...
    rs = func_taking_mean_perf("pd_single")
    %["hc_single"; "pd_single"; "cp_single";"cdp_single"]
    ...
    for ig = 1:n_group
        for il = 1:n_load                       
            ...
            runs_generic = [group(ig) + "_*_none_none_1.0_data_center_a1.0_-*txt",...
                            group(ig) + "_*_none_none_1.0_density_a1.0_-*txt",...
                            group(ig) + "_*_none_none_1.0_mean_a1.0_-*txt",...
                            group(ig) + "_*_none_data_center_linear_order_1.0_best_a0.01_-*txt",...
                            group(ig) + "_*_none_density_1.0_best_a0.01_-*txt",... 
                            group(ig) + "_*_mean_euclidian_distance_0.1_best_a0.01_-*txt",...
                            group(ig) + "_*_none_data_center_linear_order_0.25_data_center_a0.01_-*txt",...
                            group(ig) + "_*_none_density_0.25_density_a0.01_-*txt",...
                            group(ig) + "_*_mean_euclidian_distance_0.25_mean_a0.01_*txt",...
                            group(ig) + "_*_mean_euclidian_distance_0.25_data_center_a0.01_*txt",...
                            group(ig) + "_*_mean_euclidian_distance_0.25_density_a0.01_-*txt",...
                            group(ig) + "_*_data_center_euclidian_distance_0.25_data_center_a0.01_*txt"];
...
function [means_std, percentual] = test_take_mean_mpol(folder, env, env_abr, load, alg, runs, printing, steps_per_second, steps_counted, runs_number, withLimiar)
    ...
    elseif (alg == "2020ddpg")
        subfolder = env + "_mpols" + load + "_yamls_results/";
        preffix = env + "_tau_replay_ddpg_tensorflow_sincos16";
    end
