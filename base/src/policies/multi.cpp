  ...
  *pt_iterations_ = *pt_iterations_ + 1;
  ...
  switch (strategy_)
  {
    ...
    case csRoulette:
    {
      ...
      for(size_t i = 0; i < mean_mov_->size(); ++i)
      {
        CRAWL( "MultiPolicy::csRoulette::rouling::roulette: " << roulette << ", mean_mov_->at(i:" << i << "): " << mean_mov_->at(i) );
        if (roulette <= mean_mov_->at(i))
        {
          ind = i;
          break;
        }
        roulette -= mean_mov_->at(i);
      }

      CRAWL("MultiPolicy::csRoulette::updating mean_mov_::ind: " << ind);
      for(size_t i = 0; i < mean_mov_->size(); ++i)
      {
        CRAWL("MultiPolicy::csRoulette::before::mean_mov(i:" << i << "): " << mean_mov_->at(i));
        if ((i != ind) && (mean_mov_->at(i) > 0))
        {
          mean_mov_->at(ind) = mean_mov_->at(ind) + iRoulette_;
          mean_mov_->at(i) = mean_mov_->at(i) - iRoulette_;
        }
        CRAWL("MultiPolicy::csRoulette::after for::mean_mov(i:" << i << "): " << mean_mov_->at(i));
      }
      dist = actions_actors[ind].v;
    }
    break;
